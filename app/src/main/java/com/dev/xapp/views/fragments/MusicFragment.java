...
import com.dev.xapp.views.activities.AlbumsAndArtistsDetailsActivity;
...
public class MusicFragment extends Fragment {
    ...
    public static class AlbumsFragment extends Fragment {
        ...
        void loadAudioData() {
            ...
            String[] projection = {MediaStore.Audio.Media.ALBUM_ID, MediaStore.Audio.Albums.ALBUM,
                    MediaStore.Audio.Albums.NUMBER_OF_SONGS, MediaStore.Audio.Albums.ARTIST};
            cursor = contentResolver.query(uri, projection, null, null, null);
            if (cursor != null && cursor.moveToFirst())
                while (cursor.moveToNext())
                    songsList.add(new Song(cursor.getString(1), cursor.getString(3),
        ...
        static class GridViewAdapter extends BaseAdapter{
            ...
            public View getView(int position, View convertView, ViewGroup parent) {
                ...
                try {
                    byte[] bytes = Song.getAudioAlbumArt(hashMap.get(AllSongsFragment.songsList.get(position).albumId));
                    if(bytes != null)
    ...
    public static class AllSongsFragment extends Fragment {
        ...
        void loadAudioData() {
            ...
            String[] projection = {MediaStore.Audio.Media.ALBUM, MediaStore.Audio.Media.TITLE, MediaStore.Audio.Media.DURATION,
                    ...
                    MediaStore.Audio.Media.ARTIST_ID, MediaStore.Audio.Albums.ALBUM};
            cursor = contentResolver.query(uri, projection, null, null, null);
            if (cursor != null && cursor.moveToFirst())
                while (cursor.moveToNext()) {
                    songsList.add(new Song(cursor.getString(3), cursor.getString(1),
                            cursor.getString(4), cursor.getInt(2),
                            cursor.getString(5), cursor.getString(projection.length-2),
                            cursor.getString(projection.length-1)));
                }
            Objects.requireNonNull(cursor).close();
            String sortType = sharedPreferences.getString("song_sort", "name a to z");
            boolean isReverse = sharedPreferences.getBoolean("song_sort_isReverse", false);
